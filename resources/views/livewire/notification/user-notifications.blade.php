<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Notifikasi Saya</h2>
            <p class="text-sm text-gray-500 mt-1">{{ $unreadCount }} notifikasi belum dibaca</p>
        </div>
        ...
        <div class="p-4 space-y-4">
            ...
            @if(count($stats['by_type']) > 0)
                <div class="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
                    <span class="text-xs font-medium text-gray-500">Berdasarkan Tipe:</span>
                    @foreach($stats['by_type'] as $type => $count)
                        @php
                            ...
                            $badgeVariant = $badgeVariantMap[$type] ?? 'gray';
                        @endphp
    ...
    <div class="space-y-3">
        ...
        @empty
            <x-layout.empty-state 
                icon="bell" 
                :title="$search ? 'Tidak ada notifikasi yang cocok' : ($filter === 'unread' ? 'Tidak ada notifikasi belum dibaca' : ($filter === 'read' ? 'Tidak ada notifikasi yang sudah dibaca' : 'Tidak ada notifikasi'))"
                :description="$search ? 'Coba ubah kata kunci pencarian Anda' : 'Notifikasi akan muncul di sini ketika ada aktivitas baru'"
            />
        @endforelse
    </div>
