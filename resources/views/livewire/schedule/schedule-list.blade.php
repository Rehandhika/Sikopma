    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 mb-6">
        ...
        <div class="mb-4">
            <div class="flex items-center justify-between text-sm mb-1.5">
                ...
                <span class="font-semibold text-gray-900">{{ $this->availabilityStats['percentage'] }}%</span>
            </div>
            <div class="w-full bg-gray-100 rounded-full h-2">
                <div class="bg-green-500 h-2 rounded-full transition-all duration-500" style="width: {{ $this->availabilityStats['percentage'] }}%"></div>
            </div>
        </div>
        ...
        <div class="sm:hidden space-y-2 max-h-80 overflow-y-auto">
            ...
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500">{{ $member['nim'] ?? '-' }}</p>
                </div>
                <div class="flex items-center gap-3 ml-3">
                    <span class="text-sm text-gray-600">{{ $member['total_sessions'] }}/12</span>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2.5 text-sm text-gray-500">{{ $member['nim'] ?? '-' }}</td>
                        ...
                        <td class="px-4 py-2.5 text-center text-sm text-gray-600">{{ $member['total_sessions'] }}/12</td>
                        <td class="px-4 py-2.5 text-center">
                            @if($member['has_submitted'])
                                <button wire:click="viewMemberAvailability({{ $member['id'] }})" 
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        @if($schedules->isEmpty())
            <div class="text-center py-12 px-4">
                ...
                <a href="{{ route('admin.schedule.create') }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
                    + Buat Jadwal Baru
                </a>
            </div>
        @else
            ...
            <div class="sm:hidden divide-y divide-gray-100">
                ...
                <div class="p-4">
                    <div class="flex items-start justify-between mb-2">
                        <div>
                            <p class="text-sm font-medium text-gray-900">
                                ...
                                {{ \Carbon\Carbon::parse($schedule->week_end_date)->format('d M Y') }}
                            </p>
                            <p class="text-xs text-gray-500 mt-0.5">{{ $schedule->assignments_count }} assignments</p>
                        </div>
                    ...
                    <div class="flex items-center gap-2 mt-3">
                        ...
                        <button wire:click="publish({{ $schedule->id }})" 
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-900">{{ $schedule->assignments_count }} assignments</p>
                                <p class="text-xs text-gray-500">{{ number_format(($schedule->assignments_count / 12) * 100, 0) }}% coverage</p>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-900">{{ $schedule->created_at->format('d M Y') }}</p>
                                <p class="text-xs text-gray-500">{{ $schedule->created_at->diffForHumans() }}</p>
                            </td>
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-auto">
                <div class="flex items-center justify-between p-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">{{ $selectedMemberName }}</h3>
                ...
                <div class="p-4">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            @php $days = ['monday' => 'Senin', 'tuesday' => 'Selasa', 'wednesday' => 'Rabu', 'thursday' => 'Kamis']; @endphp
                            @foreach($days as $dayKey => $dayLabel)
                            <tr class="border-b border-gray-100 last:border-0">
                                <td class="py-2.5 font-medium text-gray-900">{{ $dayLabel }}</td>
                                @for($s = 1; $s <= 3; $s++)
